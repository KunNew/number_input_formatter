...
class AmountInputFormatter extends TextInputFormatter {
  ...
  int _calculateSelectionOffset({
    ...
    // Check if all text was selected (text replacement scenario)
    bool wasAllTextSelected = oldValue.selection.baseOffset == 0 && 
                              oldValue.selection.extentOffset == oldValue.text.length;
    
    // If all text was selected and we're replacing it, position cursor after the new input
    if (wasAllTextSelected && newValue.text != oldValue.text) {
      // Handle negative sign positioning
      int negativeOffset = newText.startsWith('-') ? 1 : 0;
      
      // For single digit input, place cursor after the digit (before decimal if exists)
      if (formatter.doubleValue.abs() < 10 && formatter.doubleValue.abs() >= 1) {
        return 1 + negativeOffset; // Position after the single digit
      }
      // For zero or other cases, place cursor at appropriate position
      if (formatter.doubleValue == 0) {
        return 1 + negativeOffset; // Position after the "0"
      }
      // For larger numbers, place cursor at the end of integer part
      return formatter.indexOfDot > 0 ? formatter.indexOfDot : newText.length;
    }
    ...
    // of the integer part.
    if (oldValue.selection.baseOffset <= (1 + negativeOffset) && formatter.doubleValue.abs() <= 9) {
      if (newText.isEmpty) return 0;

      if (formatter.doubleValue == 0) {
        if (formatter.previousValue == 0 &&
            formatter.ftlDigits > 0 &&
            (newValue.selection.baseOffset > (1 + negativeOffset) || oldValue.text.isEmpty)) {
    ...
    if (oldValue.selection.baseOffset <= (1 + negativeOffset) &&
